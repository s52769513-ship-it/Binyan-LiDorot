...
function DonationTab({ parent, onUpdate }: { parent: ParentDetail; onUpdate: () => void }) {
  ...
  useEffect(() => { setAmountDraft(String(parent.monthlyDonation || '')) }, [parent.monthlyDonation])
  ...
  useEffect(() => {
    const loadPPs = async () => {
      ...
      try {
        ...
        if (r.ok) { const data = await r.json(); setDonationPPs(Array.isArray(data) ? data : []) }
      } catch {} finally { setLoadingPPs(false) }
    }
    loadPPs()
    if (parent.standingOrders) {
      setDonationSOs(parent.standingOrders.filter(so => so.projectName === 'דמי מגבית').map(so => ({
        id: so.id, type: so.standingOrderType, amount: so.chargeAmount ?? 0, status: so.soStatus,
      })))
    }
  ...
  const saveAmount = async () => {
    ...
    try {
      ...
      onUpdate()
    } finally { setSaving(false); setEditingAmount(false) }
  }
    ...
    <div className="p-4 space-y-4" dir="rtl">
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${hasDonation || hasDonationSO ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            {editingAmount ? (
              <div className="flex items-center gap-1">
                <input type="number" value={amountDraft} onChange={e => setAmountDraft(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') saveAmount(); if (e.key === 'Escape') setEditingAmount(false) }}
                  autoFocus className="w-24 px-2 py-1 text-sm border border-emerald-300 rounded-lg focus:outline-none" dir="ltr" placeholder="0" />
                <button onClick={saveAmount} disabled={saving} className="px-2 py-1 text-xs bg-emerald-600 text-white rounded-lg disabled:opacity-40">{saving ? '...' : '✓'}</button>
                <button onClick={() => setEditingAmount(false)} className="text-gray-400 text-xs px-1">✕</button>
              </div>
            ) : (
              <button onClick={() => setEditingAmount(true)} className="text-xl font-bold text-emerald-700 hover:text-emerald-900 transition-colors group">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-gray-700">{paymentMethods.length > 0 ? paymentMethods.join(', ') : '—'}</p>
          </div>
          {monthlyAmount > 0 && (
          ...
          <div className="mt-3 pt-3 border-t border-emerald-200 space-y-1">
            ...
            {donationSOs.map(so => (
              <div key={so.id} className="flex items-center justify-between text-xs">
                <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${so.status === 'פעיל' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>{so.status}</span>
        ...
        <div className="mt-3 pt-3 border-t border-emerald-200 flex items-center justify-between">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={parent.deductDonation ?? false}
              onChange={async e => {
                await fetch(`/api/parents/${parent.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ deductDonation: e.target.checked }) })
                onUpdate()
                  ...
                  return (
                    <tr key={pp.id}>
                      ...
                      <td className="px-3 py-2 text-center">
                        {paid ? <span className="px-1.5 py-0.5 rounded text-[10px] bg-emerald-100 text-emerald-700">שולם ✓</span>
                          : partial ? <span className="px-1.5 py-0.5 rounded text-[10px] bg-amber-100 text-amber-700">חלקי</span>
