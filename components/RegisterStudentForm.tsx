...
/**
 * טופס רישום תלמיד. משמש גם בעמוד "רישום תלמיד" וגם כחלון בתוך לשונית הנרשמים,
 * כדי שלא יהיה טופס כפול שמתפצל.
 * `embedded` מסתיר את הכותרת (בחלון יש כותרת משלו), ו-`onSaved` מאפשר לרענן את
 * הרשימה שמאחורי החלון מיד אחרי שמירה.
 */
export default function RegisterStudentForm({ embedded = false, onSaved }: {
  embedded?: boolean
  onSaved?: () => void
} = {}) {
  ...
  return (
    <div className={embedded ? '' : 'max-w-2xl mx-auto'}>
      {!embedded && <h2 className="text-2xl font-bold text-gray-800 mb-6 text-right">רישום תלמיד חדש</h2>}
