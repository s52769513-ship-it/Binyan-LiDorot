      ...
      <div className="bg-white rounded-xl shadow-lg max-w-md w-full max-h-[28rem] flex flex-col">
        <div className="p-4 border-b">
          <p className="text-sm font-semibold text-gray-700 mb-3">
            בחר הורה עבור: <span className="text-blue-600">{unmatchedName}</span>
          </p>
          <input
            type="text"
...
export default function NedarimTransactionsPullTab() {
  ...
  // Manual mappings: rawDonorName → { id, name }
  const [manualMappings, setManualMappings] = useState<Record<string, { id: string; name: string }>>(() => {
    try { return JSON.parse(localStorage.getItem('nedarimTxManualMappings') ?? 'null') ?? {} } catch { return {} }
  })

  useEffect(() => {
    try { localStorage.setItem('nedarimTxManualMappings', JSON.stringify(manualMappings)) } catch {}
  }, [manualMappings])
  ...
  useEffect(() => {
    fetch('/api/automations/nedarim-transactions-pull')
      .then(r => r.json())
      .then(d => setLastRun(d ?? {}))
      .catch(() => {})
  }, [])
  ...
  const runPreview = async () => {
    ...
    try {
      const res = await fetch('/api/automations/nedarim-transactions-pull', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ dateFrom, dateTo, dryRun: true, parentMappings: buildParentMappings() }),
      })
  ...
  const runImport = async () => {
    if (!preview) return
    if (!confirm(`מאשר יצירת ${preview.matched} עסקאות ב-${dateFrom} עד ${dateTo}?`)) return
    setBusy(true); setResult(null)
    try {
      const res = await fetch('/api/automations/nedarim-transactions-pull', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ dateFrom, dateTo, dryRun: false, parentMappings: buildParentMappings() }),
      })
      const data = await res.json()
      if (!res.ok || data.error) { setResult({ error: data?.error ?? `שגיאה ${res.status}` }); return }
      setResult(data); setPreview(null)
      // Refresh last-run info
      fetch('/api/automations/nedarim-transactions-pull').then(r => r.json()).then(d => setLastRun(d ?? {})).catch(() => {})
    } catch (e) { setResult({ error: String(e) }) }
    finally { setBusy(false) }
  }

  // Rows visible in table: apply name filter + overlay manual mapping data
  const filteredPreview = preview?.preview.filter(r => {
    if (!nameFilter) return true
    const hay = `${r.donorName} ${r.matchedParent ?? ''} ${manualMappings[r.donorName]?.name ?? ''}`.toLowerCase()
    return hay.includes(nameFilter.toLowerCase())
  }) ?? []

  const stillUnmatched = preview?.unmatched.filter(n => !manualMappings[n]) ?? []
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
        <div>
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">משיכת תנועות מנדרים</h3>
          <p className="text-xs text-gray-400 mt-0.5">
            מושך תנועות כספיות מנדרים פלוס (לא הו&quot;ק בנקאי ולא אשראי) ומקשר לתשלומים מתוכננים של ההורה המתאים.
          </p>
          {lastRun.lastRun && (
        ...
        <div className="flex gap-4 items-end flex-wrap">
          <div>
            ...
            <input
              ...
              onChange={e => { setDateFrom(e.target.value); setPreview(null); setResult(null) }}
              className="px-3 py-2 rounded-xl border border-gray-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#1a3a7a]/30"
              dir="ltr"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={e => { setDateTo(e.target.value); setPreview(null); setResult(null) }}
              className="px-3 py-2 rounded-xl border border-gray-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#1a3a7a]/30"
              dir="ltr"
            />
          </div>
      ...
      {preview && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
          ...
          <div className="grid grid-cols-5 gap-2">
            <Stat label="סה״כ נדרים"   value={preview.total}        />
            <Stat label="הוסר (סונן)"   value={preview.excluded}     color="amber" />
            <Stat label="רלוונטיים"     value={preview.txCount}      color="blue"  />
          ...
          <p className="text-sm text-gray-600">
            סה&quot;כ סכום תנועות: <strong className="text-gray-900">₪{preview.totalAmount.toLocaleString('he-IL')}</strong>
          </p>

          {/* Endpoint used (dev info) */}
          {preview.endpoint && (
            <p className="text-[10px] text-gray-300 break-all">{preview.endpoint}</p>
          )}
          ...
          {stillUnmatched.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 space-y-2">
              <p className="text-xs font-semibold text-red-700">
                הורים שלא זוהו ({stillUnmatched.length}) — לחץ לקישור ידני:
              </p>
              <div className="flex flex-wrap gap-1.5">
                {stillUnmatched.map(name => (
                  <button
                    key={name}
                    onClick={() => setSelectorOpen(name)}
          ...
          <div>
            <input
              type="text"
              placeholder="סנן לפי שם תורם / הורה…"
              value={nameFilter}
          ...
          <div className="overflow-x-auto max-h-80 overflow-y-auto border border-gray-100 rounded-xl">
            <table className="w-full text-xs">
              <thead className="bg-gray-50 sticky top-0 text-gray-400 text-right">
                <tr>
                  <th className="px-3 py-2">שם תורם</th>
          ...
          <p className="text-[11px] text-gray-400">
            מוצגות {filteredPreview.length} מתוך {preview.txCount} שורות רלוונטיות.
          ...
          <div className="flex gap-3 pt-2 border-t border-gray-100">
            ...
            >
              {busy ? 'מייבא…' : `ייבא ${preview.matched} עסקאות`}
            </button>
          </div>
        </div>
      )}
      ...
      {result && !result.error && (
        <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm p-6 space-y-3">
          <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-sm text-emerald-800">
            ✓ נוצרו <strong>{result.created}</strong> עסקאות
            ...
            {result.excluded ? <span className="text-amber-600"> · {result.excluded} סוננו (הו&quot;ק/אשראי)</span> : null}
          </div>
      ...
      {selectorOpen && (
        <ParentSelector
          unmatchedName={selectorOpen}
          allParents={allParents}
          onSelect={(id, name) => {
            setManualMappings(m => ({ ...m, [selectorOpen]: { id, name } }))
