  ...
  try {
    ...
    const result = months.map(m => {
      ...
      return {
        ...
        donation: {
          planned: Math.round(dn.planned),
          collected: Math.round(dn.collected),
          remaining: Math.round(dn.remaining),
          collectionPct: dn.planned > 0 ? Math.round((dn.collected / dn.planned) * 100) : 0,
        },
        net: Math.round(t.planned + dn.planned - s.planned),
        netActual: Math.round(t.collected + dn.collected - s.paid),
      }
    })
