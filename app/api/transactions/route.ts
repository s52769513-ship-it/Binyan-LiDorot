  ...
  try {
    ...
    if (parentIdFilter !== null) {
      if (parentIdFilter.length === 0) {
        // No matching parents → return empty
        return NextResponse.json({ data: [], total: 0, months: [], types: [] })
      }
    ...
    // Fetch distinct months and types for filter dropdowns
    const [{ data: allMonths }, { data: allTypes }] = await Promise.all([
      supabaseAdmin.from('transactions').select('month_year').not('month_year', 'is', null),
    ...
    return NextResponse.json({ data: rows, total: count ?? 0, months, types })
  } catch (err) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { amount, type, date, monthYear, notes, parentIds } = body
